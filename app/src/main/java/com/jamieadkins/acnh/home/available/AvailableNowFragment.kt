...
class AvailableNowFragment : DaggerFragment(), AvailableNowContract.View {

    private var binding: FragmentCritterListBinding? = null
    @Inject lateinit var presenter: AvailableNowContract.Presenter
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding?.toolbar?.setTitle(R.string.currently_available)
    ...
    override fun showCrittersAvailableNow(bugFishSummary: BugFishSummaryEntity) {
        groupAdapter.update(mapCrittersToItems(bugFishSummary))
    }

    private fun mapCrittersToItems(critters: BugFishSummaryEntity): List<Item> {
        return critters.fish.map { FishItem(it) {} } + critters.bugs.map(::BugItem)
    }
}
